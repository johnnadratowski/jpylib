""" PhotoUpload class is for common functionality in uploading photos. """
import os
import StringIO
...
class PhotoUpload():
    ...
    def resize_to_value(self, image, value=200):
        """ Shrink down this image equally.  This method shrink an image base.
        """
        (actual_width, actual_height) = image.size
        if actual_width > actual_height:
            # Landscape
            width = value
            width_difference_percentage = float(width)/float(actual_width)
            height = actual_height * width_difference_percentage
        else:
            # Portrait or Square Photo
            height = value
            height_difference_percentage = float(height)/float(actual_height)
            width = actual_width * height_difference_percentage
        resized_image = self.resize_image(image, width=width, height=height)
        return resized_image
